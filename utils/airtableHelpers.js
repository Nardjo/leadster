...
const AIRTABLE_BASE_ID =
	process.env.AIRTABLE_BASE_ID || "YOUR_AIRTABLE_BASE_ID";
...
const AIRTABLE_REQUEST_TIMEOUT = process.env.AIRTABLE_REQUEST_TIMEOUT
	? Number.parseInt(process.env.AIRTABLE_REQUEST_TIMEOUT)
...
export async function uploadToAirtable(data) {
	try {
		...
		if (error.statusCode === 401 || error.error === "AUTHENTICATION_REQUIRED") {
			...
			console.error(
				"Not found error: Please check your Airtable Base ID and Table name",
			);
		} else if (
			error.statusCode === 413 ||
			error.error === "REQUEST_TOO_LARGE"
		) {
			...
			console.error(
				"Rate limit exceeded: Try again later or reduce the frequency of requests",
			);
...
export async function fetchRecordsByStatus(statusFieldName, statusValue) {
	try {
		...
		console.log(
			`Fetching records with ${statusFieldName} = "${statusValue}" from Airtable...`,
		);
		...
		console.log(
			`Fetched ${records.length} records with ${statusFieldName} = "${statusValue}" from Airtable.`,
		);
		...
		console.error(
			`Error fetching records with ${statusFieldName} = "${statusValue}" from Airtable:`,
			error.message,
		);
