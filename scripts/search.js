...
function isExcludedBrand(tags) {
	...
	for (const field of fields) {
		...
		if (EXCLUDED_BRANDS.some((b) => lower.includes(b.toLowerCase())))
			return true;
	}
	return false;
}
...
async function queryOverpassAPI() {
	...
	// ===== AREAS =====
	const areaParts = SEARCH_AREAS.map(
...
out body;>;out skel qt;`;
	try {
		...
		for (const e of data.elements) {
			...
			if (!typeLbl) continue;
			const website =
				e.tags.website || e.tags["contact:website"] || e.tags.url || null;
...
async function main() {
	...
	const tasks = shopsWithWebsite.map((shop) =>
		limit(async () => {
			if (
				seen.has(`${shop.website}|${shop.type}`) ||
				isShopInAirtable(
					{ URL_Site: shop.website, Type_Commerce: shop.type },
					airtable,
				)
			)
				return null;
			if (shop.igTag) {
				// IG tag présent dans OSM
				return {
					Nom: shop.igTag,
					URL_Site: shop.website,
					Ville: shop.city || shop.postcode,
	...
	const unique = results.filter(
		(r) =>
			...
			!prev.some(
				(p) =>
					p.Nom === r.Nom ||
					(p.URL_Site === r.URL_Site && p.Type_Commerce === r.Type_Commerce),
			) &&
			// Not in Airtable
